...
def vcf2hdf5(fname, output_prefix=None, mode=default_mode,
    ...
    mode : str
        ...
        In 'inbred' mode, the resulting 1D arrays are haplotypes (0,1
        and -1).  Heterozygote calls are treated as missing data.
    ...
    for r in regions:
        ...
        for s in v(r):
            if len(s.ALT) > 1 or s.aaf < maf or not keepvar[s.ID]:
                continue
            snps.append(s)
        f5[f"variants/{r}"].create_dataset('ID',
                                           (len(snps), ),
                                           dtype=h5py.string_dtype())
        f5[f"variants/{r}"].create_dataset('CHROM',
                                           (len(snps), ),
                                           dtype=h5py.string_dtype())
        f5[f"variants/{r}"].create_dataset('POS',
                                           (len(snps), ),
                                           dtype=np.uint32)
        f5[f"variants/{r}"].create_dataset('REF',
                                           (len(snps), ),
                                           dtype='S20')
        f5[f"variants/{r}"].create_dataset('ALT',
                                           (len(snps), ),
                                           dtype='S20')
        if mode == 'likelihood':
            f5["genotypes"].create_dataset(f'{r}',
                                           (len(smp), 3, len(snps)),
                                           chunks=(
                                               min(50, len(smp)),
                                               3,
        ...
        for i, sid in enumerate(smp):
            if mode == 'likelihood':
                ...
                if mode == 'genotype':
                    f5['genotypes'][f"{r}"][i, ] = np.array(
                        [geno2int(*g[:2]) for g in geno], dtype=np.int8)
                elif mode == 'inbred':
                    f5['genotypes'][f"{r}"][i, ] = np.array(
                        [geno2hap(*g[:2]) for g in geno], dtype=np.int8)
                elif mode == 'phased':
                    for si, g in geno:
                        if g[2]:

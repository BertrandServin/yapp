...
class ChromosomePair():
    ...
    @classmethod
    def from_phase_data(cls, genotype,
                        paternal_hap, maternal_hap,
                        paternal_si, maternal_si):
        obj = cls(genotype)
    ...
    def get_segregation_indicators(self, gam, recmap, err=1e-2):
        ...
        for m in range(self.len):
            if (
                    (gam.haplotype[m] > -1) and
                    self.g[m] > -1 and
                    self.H[0].haplotype[m] > -1 and
                    self.H[1].haplotype[m] > -1
            ):
                # ignore mendelian errors
                if (
                        (gam.haplotype[m] == 0 and self.g[m] == 2) or
                    ...
                    emissions[m, 0] = (
                        gam.haplotype[m] == self.paternal_gamete.haplotype[m]) and (1-err) or err # noqa
                    emissions[m, 1] = (
                        gam.haplotype[m] == self.maternal_gamete.haplotype[m]) and (1-err) or err # noqa
        ...
        for m in range(1, self.len):
            fwd[m, 0] = (fwd[m-1, 0]*transitions[m-1, 0, 0] +
                         fwd[m-1, 1]*transitions[m-1, 1, 0])*emissions[m, 0]
            fwd[m, 1] = (fwd[m-1, 0]*transitions[m-1, 0, 1] +
                         fwd[m-1, 1]*transitions[m-1, 1, 1])*emissions[m, 1]
            sca[m] = 1.0/(fwd[m, 0]+fwd[m, 1])
            fwd[m, ] *= sca[m]
        # Compute backward probabilities
        rew[self.len-1, ] /= sca[m-1]
        for m in range(self.len-1, 0, -1):
            rew[m-1, 0] = transitions[m-1, 0, 0]*emissions[m, 0] * \
                rew[m, 0] + transitions[m-1, 0, 1]*emissions[m, 1]*rew[m, 1]
            rew[m-1, 1] = transitions[m-1, 1, 0]*emissions[m, 0] * \
                rew[m, 0] + transitions[m-1, 1, 1]*emissions[m, 1]*rew[m, 1]
            rew[m-1, ] *= sca[m-1]
        ...
        # recursion
        for m in range(1, self.len):
            val_0 = [delta[m-1, 0]+np.log(transitions[m-1, 0, 0]),
                     delta[m-1, 1]+np.log(transitions[m-1, 1, 0])]
            val_1 = [delta[m-1, 0]+np.log(transitions[m-1, 0, 1]),
    ...
    def update_unknown_gamete(self, prop_gam, err, q=1e-3):
        ...
        """
        misses_pat, new_gam_p = gamete.Gamete.combine(
            self.paternal_gamete, prop_gam)
        misses_mat, new_gam_m = gamete.Gamete.combine(
            self.maternal_gamete, prop_gam)
    ...
    def update_gamete(self, prop_gam, parent):
        ...
        if parent == 0:
            ...
            raise ValueError(
                f"{type(self)}:[update_gamete]: parent must be 0 or 1")
        misses_origin, new_gam = gamete.Gamete.combine(
            self.H[origin], prop_gam)
        nmiss[origin] = len(misses_origin)
        self.H[origin] = new_gam
        prop_gam_o = gamete.Gamete.complement(self.H[origin], self.g)
        misses_other, new_gam = gamete.Gamete.combine(
            self.H[1-origin], prop_gam_o)
...
class Phaser():
    def __init__(self, vcf_file, ped_file, out_prfx,
                 region=None, focalID=None, err=1e-3, rho=1):
    ...
    def from_prefix(cls, prfx, region=None, focalID=None, err=1e-3):
        ...
        **kwargs : arguments
        ...
        obj = cls(vcf_file, fam_file, prfx,
                  region=region, focalID=focalID, err=err)
        print(obj.data.tree())
        return obj
    ...
    def write_phased_vcf(self):
        ...
        for s in vcf_tmpl:
            snp_mapping[s.ID] = s
        nidx = np.array([vcf_tmpl.samples.index(n.indiv)
    ...
    def run(self, ncpu=cpu_count()):
        ...
        for reg in self.regions:
            ...
            try:
                phases, ignored = self.phase_samples_from_genotypes(
                    reg, ncpu=ncpu)
            except KeyError:
            ...
            seg_probs = np.full(shape=(nind, 2, nsnp),
                                dtype='float', fill_value=0.5)
            for i, name in enumerate(self.data['samples']):
                try:
                    chpair = phases[name]
                except KeyError:
        ...
        with Pool(cpu_count()) as workers:
            for indiv, segind in workers.imap_unordered(run_segregation_task,
                ...
                if maxprob < 0.999:
                    ...
                    new_gam = chpair_p.get_transmitted_from_segregation(
                        chpair.si_pat)
                    nmiss = chpair.update_paternal_gamete(new_gam)
                    if (nmiss[0]+nmiss[1]) > qerr(chpair.nhet*2,
                                                  self.err,
                                                  q=1e-3/(len(pat_seg_tasks)+len(mat_seg_tasks))): # noqa
                        logger.warning(
                            f"[from_seg] {node.father.indiv}[pat] -> {node.indiv} :" # noqa
                            f"{50*(nmiss[0]+nmiss[1])/chpair.nhet:.1g} % mismatch") # noqa
                        ignore_child[node.indiv] = True
            ...
            for indiv, segind in workers.imap_unordered(run_segregation_task,
                ...
                if maxprob < 0.999:
                    ...
                    new_gam = chpair_m.get_transmitted_from_segregation(
                        chpair.si_mat)
        ...
        with Pool(ncpu) as workers:
            for indiv, new_gam in workers.imap(wcsp_phase, wcsp_tasks):
                ...
                try:
                    ...
                    logger.debug(f"Inconsistency in phase data "
                                 f"for individual {indiv} on region {region}. "
